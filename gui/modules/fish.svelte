<script>
  ...
  const entity = 'smartyellow/fish';
  const moduleName = 'smartyellow/fish';
  const pluginName = 'smartyellow/aqua';
  const readonly = user.cannot(pluginName + '/editFish');
  ...
  onMount(async function () {
    try {
      if (id) {
        ...
        settings = await api.get('/aqua/fish/settings');
        // get item, form and log for specified id
        ({ item, form, log } = isNew ? await api.post('/aqua/fish', {}, { init: true }) : await api.get('/aqua/fish/' + id));
        ...
        filters = await api.get('/aqua/fish/filters');
        gridOptions.columns = await api.get('/aqua/fish/formats');
      ...
      console.log('Failed to fetch fish', e);
    }
  });
  ...
  async function saveChanges() {
    if (focused) {
      // save changes for fish in form
      savebar.start();
      const result = isNew ?
        await api.post('/aqua/fish', item) :
        await api.put('/aqua/fish/' + item.id, item);
      savebar.stop(result);
      if (!result.errors) {
        log = result.log;
        // Once saved, we need to use PUT request to update newly created fish
        isNew = false;
        // Update tab title
        dispatch('tabchanged', { title: item.name });
      }
  ...
  async function submitFilters({ detail }) {
    ...
    items = await api.post('/aqua/fish/search', detail);
  }
  ...
  async function deleteItem() {
    if (item && item.id) {
      try {
        ...
        dispatch('close');
        await api.delete('/aqua/fish/' + item.id);
      }
      catch (e) {
        console.log(e);
      }
...
{#if mounted}
  ...
  {#if item}
    ...
    <Toolbar large="768" fill="true">
      {#if user.can(pluginName + '/editFish')}
      ...
      {/if}
      {#if log && log.created && user.can(pluginName + '/deleteFish')}
  ...
  {:else}
    ...
    <Toolbar large="768">
      ...
      />
      {#if user.can(pluginName + '/createFish')}
        <ToolbarButton {icon} on:click={e => openItem(e, true)} title={translate('new', language)} hint={translate('Create new fish', language)} />
      {/if}
    </Toolbar>
