<script>
  ...
  const entity = 'smartyellow/aquariummaintenance';
  const moduleName = 'smartyellow/aquariummaintenance';
  const pluginName = 'smartyellow/aqua';
  const readonly = user.cannot(pluginName + '/editMaintenance');
  ...
  onMount(async function () {
    try {
      if (id) {
        ...
        settings = await api.get('/aqua/maintenance/settings');
        // get item, form and log for specified id
        ({ item, form, log } = isNew ? await api.post('/aqua/maintenance', {}, { init: true }) : await api.get('/aqua/maintenance/' + id));
        // if existing item, set tabtitle to title of item
        if (!isNew) {
          dispatch('tabchanged', { title: item.date });
        }
      }
      else {
        filters = await api.get('/aqua/maintenance/filters');
        gridOptions.columns = await api.get('/aqua/maintenance/formats');
        // subscribe to 'news reload' message
      ...
      console.log('Failed to fetch maintenance', e);
    }
  });
  ...
  async function saveChanges() {
    if (focused) {
      // save changes for maintenance in form
      savebar.start();
      const result = isNew ?
        await api.post('/aqua/maintenance', item) :
        await api.put('/aqua/maintenance/' + item.id, item);
      savebar.stop(result);
      if (!result.errors) {
        log = result.log;
        // Once saved, we need to use PUT request to update newly created maintenance
        isNew = false;
        // Update tab title
        dispatch('tabchanged', { title: item.date });
      }
  ...
  async function submitFilters({ detail }) {
    ...
    items = await api.post('/aqua/maintenance/search', detail);
  }
  ...
  async function deleteItem() {
    if (item && item.id) {
      try {
        ...
        dispatch('close');
        await api.delete('/aqua/maintenance/' + item.id);
      }
      catch (e) {
        console.log(e);
      }
...
{#if mounted}
  ...
  {#if item}
    ...
    <Toolbar large="768" fill="true">
      {#if user.can(pluginName + '/editMaintenance')}
      ...
      {#if form && form.languages}
        <ToolbarButton icon={icons.lexicon} submenu title="{translate('language', language)}">
          {#each form.languages as l}
            <li
              class:checked={l.enabled}
              on:click={() => (l.enabled = !l.enabled)}>
              {l.name}
            </li>
          {/each}
        </ToolbarButton>
      {/if}
      {#if log && log.created}
        <ToolbarButton icon={icons.history} bind:value={history} hint="History" title="{translate('history', language)}" />
      {/if}
      {#if log && log.created && user.can(pluginName + '/deleteMaintenance')}
        <ToolbarButton icon={icons.trash} on:click={deleteItem} hint="Delete" title="{translate('delete', language)}" />
      {/if}
      {#if user.is('sysadmin')}
        <ToolbarButton icon={icons.wrench} bind:value={debug} hint="Show debugger" title="{translate('debug', language)}" />
      {/if}
    </Toolbar>
  ...
  {:else}
    ...
    <Toolbar large="768">
      ...
      />
      {#if user.can(pluginName + '/createMaintenance')}
        <ToolbarButton {icon} on:click={e => openItem(e, true)} title={translate('new', language)} hint={translate('Create new maintenance', language)} />
      {/if}
    </Toolbar>
