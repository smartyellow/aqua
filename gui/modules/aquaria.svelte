<script>
  ...
  const entity = 'smartyellow/aquarium';
  const moduleName = 'smartyellow/aquaria';
  const pluginName = 'smartyellow/aqua';
  const readonly = user.cannot(pluginName + '/editAquaria');
  ...
  onMount(async function () {
    try {
      if (id) {
        ...
        settings = await api.get('/aqua/aquaria/settings');
        // get item, form and log for specified id
        ({ item, form, log } = isNew ? await api.post('/aqua/aquaria', {}, { init: true }) : await api.get('/aqua/aquaria/' + id));
        ...
        filters = await api.get('/aqua/aquaria/filters');
        gridOptions.columns = await api.get('/aqua/aquaria/formats');
      ...
      console.log('Failed to fetch aquaria', e);
    }
  });
  ...
  async function saveChanges() {
    if (focused) {
      // save changes for aquarium in form
      savebar.start();
      const result = isNew ?
        await api.post('/aqua/aquaria', item) :
        await api.put('/aqua/aquaria/' + item.id, item);
      savebar.stop(result);
      if (!result.errors) {
        log = result.log;
        // Once saved, we need to use PUT request to update newly created aquarium
        isNew = false;
        // Update tab title
        dispatch('tabchanged', { title: item.name });
      }
  ...
  async function submitFilters({ detail }) {
    ...
    items = await api.post('/aqua/aquaria/search', detail);
  }
  ...
  async function deleteItem() {
    if (item && item.id) {
      try {
        ...
        dispatch('close');
        await api.delete('/aqua/aquaria/' + item.id);
      }
      catch (e) {
        console.log(e);
      }
...
{#if mounted}
  ...
  {#if item}
    ...
    <Toolbar large="768" fill="true">
      {#if user.can(pluginName + '/editAquaria')}
      ...
      {/if}
      {#if log && log.created && user.can(pluginName + '/deleteAquaria')}
  ...
  {:else}
    ...
    <Toolbar large="768">
      ...
      />
      {#if user.can(pluginName + '/createAquaria')}
        <ToolbarButton {icon} on:click={e => openItem(e, true)} title={translate('new', language)} hint={translate('Create new aquarium', language)} />
      {/if}
    </Toolbar>
